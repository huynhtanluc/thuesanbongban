...
class UpdateKhachHangRequest extends FormRequest
{
    ...
    {
        return [
            'id'            => 'required|numeric|exists:khach_hangs,id',
            'ho_va_ten'     => 'required|string|min:5|max:100',
            'email'         => 'required|email|unique:khach_hangs,email,' . $this->id,
            'so_dien_thoai' => 'required|numeric|digits:10|unique:khach_hangs,so_dien_thoai,' . $this->id,
            'ngay_sinh'     => 'required|date|before:today',
            'gioi_tinh'     => 'required|numeric|in:0,1',
            'chung_minh_thu'=> 'required|numeric|digits_between:9,12|unique:khach_hangs,chung_minh_thu,' . $this->id,
    ...
    {
        return [
            'id.required'               => 'Khách hàng không được để trống',
            'id.numeric'                => 'Khách hàng phải là số',
